        ...
        if isData():
            if 1:
                ...
                if c == "\x1b" or c == "q" or c == "Q":
                    tmp = input("You want to quit? Y/N:\n")
                    if tmp == "Y" or tmp == "y":
                        running = False
                        break
                if c == "\x20":
                    radio.printDetails()
                    print(
                        sorted(stat.items(), key=operator.itemgetter(1), reverse=True)[
                            :10
                        ]
                    )
                ...
                print(
                    "Channel = %s, Address = %s Data Rate = %s AutoAck = %s"
                    % (
                        channel,
                        hex(address[address_index]),
                        data_rate[data_rate_index],
                        aack,
                    )
                )
