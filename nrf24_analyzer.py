...
def color(string, color="grey"):
    if color in color_base:
        return color_base[color] % string
    else:
        return color_base["grey"] % string
    ...
    while True:
        ...
        if len(p_buff) >= 329:
            ...
            if (preamb == preamb_def1) or (preamb == preamb_def2):
                ...
                # 				print p5_len
                p5_id = make_byte(
                    [0, 0, 0, 0, 0, 0] + buf3[addr_len * 8 + 6 : (addr_len + 1) * 8]
                )
                p5_ack = make_byte(
                    [0, 0, 0, 0, 0, 0, 0]
                    + buf3[(addr_len + 1) * 8 : (addr_len + 1) * 8 + 1]
                )
                ## data
                for hh in range(0, 33):
                    ...
                    for k in range(pay_len):
                        p2_data = p2_data + chr(
                            make_byte(buf3[(addr_len + k) * 8 : (addr_len + 1 + k) * 8])
                        )
                    ## crc
                    p_crc8 = make_byte(
                        buf3[
                            (p5_len_c + addr_len + 1) * 8
                            + 1 : (p5_len_c + addr_len + 2) * 8
                            + 1
                        ]
                    )
                    p_crc = (p_crc8 << 8) + make_byte(
                        buf3[
                            (p5_len_c + addr_len + 2) * 8
                            + 1 : (p5_len_c + addr_len + 3) * 8
                            + 1
                        ]
                    )
                    p2_crc8 = make_byte(
                        buf3[(addr_len + pay_len) * 8 : (addr_len + pay_len + 1) * 8]
                    )
                    p2_crc = (p2_crc8 << 8) + make_byte(
                        buf3[
                            (addr_len + pay_len + 1) * 8 : (addr_len + pay_len + 2) * 8
                        ]
                    )
                    ## calc crc8
                    crc8 = 0xFF
